...
public class WeatherStationsController : ControllerBase
{
    ...
    private static object GetValueByField(weather_stations_today a, List<string>? fields)
    {
    ...
    public static dynamic MergeObjects(dynamic obj1, dynamic obj2)
    {
        var dict1 = (IDictionary<string, object>)obj1;
        var dict2 = (IDictionary<string, object>)obj2;

        var resultDict = new Dictionary<string, object>(dict1);
    ...
    public static IEnumerable<dynamic> MergeAllTables(List<IEnumerable<dynamic>> tables)
    {
        ...
        foreach (var table in tables)
        {
            ...
            {
                var existingItem = mergedData.FirstOrDefault(x => x.station_id == item.station_id && x.station_name == item.station_name);
                ...
                else
                {
                    ...
                    {
                        if (!((IDictionary<string, object>)existingItem).ContainsKey(property.Key))
                        {
                            ((IDictionary<string, object>)existingItem).Add(property.Key, property.Value);
                        }
                    }
                }
            }
