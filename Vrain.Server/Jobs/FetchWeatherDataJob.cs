...
public class FetchWeatherDataJob : IJob
{
    ...
    public async Task FetchWeatherData()
    {
        if (_isFirstRun)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var firstRecord = _context.monitoring_data_today.FirstOrDefault();
                    ...
                    var stationData = (from tskt in _context.iw_thongsoquantrac.Where(s=>s.tskt_maloaithongso == "RAIN")
                                       ...
                                       }).ToList(); // Thực hiện truy vấn trước và lưu kết quả vào bộ nhớ
        ...
        {
            _logger.LogInformation("FetchWeatherDataJob has already run. Skipping execution.");
        }
    }
